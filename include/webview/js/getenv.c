...
SeedValue ghtml_webview_js_getenv (SeedContext ctx, SeedObject function, SeedObject thisObject, size_t argumentCount, SeedValue arguments[], SeedException * exception) {

	if (argumentCount != 1) {
		seed_make_exception (ctx, exception, GHTML_JS_INVALID_PARAMS,
			"getenv expected 1 argument, got %zd", argumentCount
		);  return seed_make_null (ctx);
	}

	gchar * var = seed_value_to_string(ctx, arguments[0], exception);
	gchar * val = (void * )g_getenv(var);
